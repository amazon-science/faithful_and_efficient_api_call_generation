...
PREFIX_TENSOR = torch.tensor([[PREFIX]]).to(device)
SUFFIX = tokenizer.convert_tokens_to_ids(']')
SUFFIX_TENSOR = torch.tensor([[SUFFIX]]).to(device)
...
OPEN_TO_CLOSE = {OPEN_PARA:[CLOSE_PARA_COMMA],
                 OPEN_STR_LIST:[CLOSE_STR_LIST],
                 OPEN_DICT_LIST2:[CLOSE_DICT_LIST]
                 }

OPEN_PARA_LIST = list(OPEN_TO_CLOSE.keys())

EOF_TO_CLOSE_PARA = {CLOSE_PARA_RIGHT_BRACKET:CLOSE_PARA_COMMA,
                     }

EOF_LIST = [RIGHT_BRACKET,CLOSE_PARA_RIGHT_BRACKET]

api_doc = pk.load(open(os.path.join(args.constraints_path,'api_constraints-tree.pk'),'rb'))
func_pv = pk.load(open(os.path.join(args.constraints_path,'api_constraints-func_pv-tree.pk'),'rb'))
...
def bfs_constrained_beam_search_unified(initial_beams, beam_size, input_length, max_length):
        ...
        for beam, node, meta, model_kwargs in current_level_beams:
            ...
            if beam.size()[-1]-input_length > max_length:
                ...
                if last_output_id==LEFT_BRACKET:
                    try:
                        termination_flag = False
                        node = api_doc[meta['function_name']]['first_para_gen_options']
                    except:
                        beam = torch.cat([beam, torch.tensor([[RIGHT_BRACKET]]).to(device)], dim=-1)

                elif last_output_id in OPEN_PARA_LIST:
                    
                    termination_flag = False

                    if last_output_id==OPEN_PARA:
                        parameter_name = tokenizer.decode(beam[0][input_length:-1]).split('(')[-1].split(', ')[-1]
                    elif last_output_id==OPEN_STR_LIST:
                        parameter_name = tokenizer.decode(beam[0][input_length:-2]).split('(')[-1].split(', ')[-1]
                    elif last_output_id==OPEN_DICT_LIST2:
                        parameter_name = tokenizer.decode(beam[0][input_length:-3]).split('(')[-1].split(', ')[-1]
                                    
                    meta['decoded_para'].append(parameter_name)

                    parameter_with_pv = api_doc[meta['function_name']]['para_pv_ids']
                    
                    meta['closing_symbols'] = OPEN_TO_CLOSE[last_output_id]
                ...
                elif last_output_id in meta['closing_symbols']:
                    
                    termination_flag = False
                    
                    if last_output_id == CLOSE_PARA_COMMA:
                        required_para = api_doc[meta['function_name']]['required_para']
                        if len(set(required_para) - set(meta['decoded_para']))==0:
                            beam[0][-1] = torch.tensor([[CLOSE_PARA_RIGHT_BRACKET]]).to(device)
                        else:
                            node=api_doc[meta['function_name']]['para_gen_options']
                    else:
                        node={CLOSE_PARA_COMMA:{}, CLOSE_PARA_RIGHT_BRACKET:{}}
                        meta['closing_symbols'] = [CLOSE_PARA_COMMA]
                elif last_output_id == CLOSE_PARA_RIGHT_BRACKET:
                    required_para = api_doc[meta['function_name']]['required_para']
                    if len(set(required_para)-set(meta['decoded_para']))>0:
                        termination_flag = False
                        beam[0][-1] = torch.tensor(EOF_TO_CLOSE_PARA[last_output_id]).to(device)
                        node = api_doc[meta['function_name']]['para_gen_options']                        
                    else:
                        next_level_beams.append((beam, node, meta, model_kwargs))
                        continue
                elif last_output_id == RIGHT_BRACKET:
                    required_para = api_doc[meta['function_name']]['required_para']
                    if len(set(required_para)-set(meta['decoded_para']))>0:
                        termination_flag = False
                        beam = beam[:,:-1]
            ...
            if node:
                ...
                if last_output_id in meta['closing_symbols']+EOF_LIST:
    ...
    return [beam[:,input_length-1:] for beam, _, _, _ in current_level_beams]
...
def constrained_beam_search(input_text, beam_size, max_length):
    ...
    meta = {'score':0, 'function_name':None, 'decoded_para':[], 'closing_symbols':[]}
    input_length = input_ids.size()[-1]

    function_beams = bfs_constrained_beam_search_decoding_of_a_unit(input_ids, func_pv, meta, beam_size=beam_size, model_kwargs={})
       
    for beam, meta, model_kwargs in function_beams:
        if not meta['function_name']:
            function_name = tokenizer.decode(beam[0][input_length:]).split('(')[0]
            meta['function_name'] = function_name
    
    initial_beams = [(pfb[0],None,pfb[1],pfb[2]) for pfb in function_beams]
    
    beams = bfs_constrained_beam_search_unified(initial_beams, beam_size, input_length, max_length)
    
    beams = [torch.cat([beam, SUFFIX_TENSOR], dim=-1) for beam in beams]
        
    return beams

def topkp_constrained_decoding_of_a_unit(output_ids, node, k=None, model_kwargs = {}):
    global time_to_subtract
    next_candidates = list(node.keys())
    unit = []
    while len(next_candidates)>0:
        if len(next_candidates)==1:
            ...
            torch.cuda.synchronize()
            time_to_subtract += time.perf_counter()-timepoint
            # <<< This inference step can be removed for packages supporting partial past_key_values. Added estimated_time_savings for the compatibility and efficiency of experimenting with HF transformers package
...
def constrained_topkp_sampling(input_ids, k=None, p=0.9, max_length=200, input_length=None):
    ...
    output_ids = torch.cat([output_ids, PREFIX_TENSOR], dim=-1) 
    ...
    function_name = tokenizer.decode(function_unit).split('(')[0].strip()

    decoded_para = []
    required_para = api_doc[function_name]['required_para']
    parameter_with_pv = api_doc[function_name]['para_pv_ids']
    ...
    while last_output_id not in [SUFFIX_TENSOR,tokenizer.eos_token_id]:
        if output_ids.size()[-1]-input_length >= max_length:
            break

        if last_output_id==LEFT_BRACKET:
            try:
                gen_options = api_doc[function_name]['first_para_gen_options']
                output_ids, unit, model_kwargs = topkp_constrained_decoding_of_a_unit(output_ids, gen_options, k=k, model_kwargs=model_kwargs)
            except:
                output_ids = torch.cat([output_ids, torch.tensor([[RIGHT_BRACKET]]).to(device)], dim=-1)
                output_ids = torch.cat([output_ids, torch.tensor([[SUFFIX_TENSOR]]).to(device)], dim=-1)
        elif last_output_id in OPEN_PARA_LIST:
            if last_output_id==OPEN_PARA:
                parameter_name = tokenizer.decode(unit[:-1])
            elif last_output_id==OPEN_STR_LIST:
                parameter_name = tokenizer.decode(unit[:-2])
            elif last_output_id==OPEN_DICT_LIST2:
                parameter_name = tokenizer.decode(unit[:-3])
            
            closing_symbols = OPEN_TO_CLOSE[last_output_id]
            ...
            if parameter_name in parameter_with_pv:
                ...
                while last_output_id not in closing_symbols+EOF_LIST:
                    model_inputs = model.prepare_inputs_for_generation(output_ids, **model_kwargs)
                    ...
                    if k:
                        ...
                        next_token_id = topk_idxs[torch.multinomial(topk_probs,1)]
                    else:
                    ...
                    if output_ids.size()[-1]-input_length >= max_length:
                        break
        elif last_output_id in closing_symbols:
            if last_output_id == CLOSE_PARA_COMMA:
                if len(set(required_para) - set(decoded_para))==0:
                    output_ids[0][-1] = torch.tensor([[CLOSE_PARA_RIGHT_BRACKET]]).to(device)
                else:
                    gen_options=api_doc[function_name]['para_gen_options']
                    output_ids, unit, model_kwargs = topkp_constrained_decoding_of_a_unit(output_ids, gen_options, k=k, model_kwargs=model_kwargs)
            else:
                gen_options={CLOSE_PARA_COMMA:{}, CLOSE_PARA_RIGHT_BRACKET:{}}
                closing_symbols = [CLOSE_PARA_COMMA]
                output_ids, unit, model_kwargs = topkp_constrained_decoding_of_a_unit(output_ids, gen_options, k=k, model_kwargs=model_kwargs)
        elif last_output_id == CLOSE_PARA_RIGHT_BRACKET:
            if len(set(required_para) - set(decoded_para))>0:
                output_ids[0][-1] = torch.tensor(EOF_TO_CLOSE_PARA[last_output_id]).to(device)
                new_options = api_doc[function_name]['para_gen_options']
                output_ids, unit, model_kwargs = topkp_constrained_decoding_of_a_unit(output_ids, new_options, k=k, model_kwargs=model_kwargs)
            else:
                output_ids = torch.cat([output_ids, SUFFIX_TENSOR], dim=-1)
        elif last_output_id == RIGHT_BRACKET:
            if len(set(required_para) - set(decoded_para))>0:
                output_ids = output_ids[:,:-1]
                new_options = api_doc[function_name]['first_para_gen_options']
                output_ids, unit, model_kwargs = topkp_constrained_decoding_of_a_unit(output_ids, new_options, k=k, model_kwargs=model_kwargs)
            else:
                output_ids = torch.cat([output_ids, SUFFIX_TENSOR], dim=-1)
